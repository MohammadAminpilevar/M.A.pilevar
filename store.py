...
def Addproduct ():
    while True:
        check=input("agar mikhahid ezafe konid mahsol ra dar id benevisid (y)  ya agar na benevisid (n)   :")
        if check=="y":
...
def Editproduct():
    while True:
        check=input("agar mikhahid edit konid dar id benevisid (y)  ya agar na benevisid (n)    :")
        if check=="y":
            i=0
            ...
            for i in range(len(PRODUCT)):
                if edit==PRODUCT[i]["name"] or PRODUCT[i]["code"]:
                    break
            edit=input("kodam  ra mikhahid edit konid benevisid alamat ra (,) dar akhar har gozine   :")
            edit1=[]
            edit1=edit.split(",")
...
def Deleteproduct():
    while True:
        check=input("agar mikhahid hazf konid dar id benevisid (y)  ya agar na benevisid (n)    :")
        if check=="y":
            i=0
            for i in range(len(PRODUCT)):
                print(PRODUCT[i]["code"] , ["name"])
...
def Searchproduct():
        while True:
            check=input("agar mikhahid search konid  dar id benevisid (y)  ya agar na benevisid (n)    :") 
            if check=="y":
...
#Show list
def Showlist():
    check=input("agar mikhahid list ra bebinid benevisid (y)  ya agar na benevisid (n)    :")
    if check=="y":
...
def Buy():
    while True:
        check=input("agar mihahid kharid konid dar id benevisid (y)  ya agar na benevisid (n)    :")
        if check=="y":
            for i in range(len(PRODUCT)):
            ...
            while True:
                ...
                count=int(input("cheghadr  mikhahid az in mahsil kharid konid   :"))
                if int(count1)>=count:
                    PRODUCT[i]["count"]=int(count1)-count
                    print("kharid kamel shod")
                    break
                else:
...
while True:
    ...
    print(f.renderText("Mohammad Store"))
    show_menu()
